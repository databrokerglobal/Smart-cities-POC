...
<html>
    ...
    <?php $site_configurations = \DB::table('site_configurations')->where('status',1)->first();?>
        ...
        <meta http-equiv="Content-Security-Policy" content="default-src *; script-src 'self'
         'unsafe-inline' 'unsafe-eval' *; style-src	'self' 'unsafe-inline' *; img-src 'self' blob: data:;">
        ...
        <link rel="icon" type="image/png" href="{{ asset('uploads/logo/'.$site_configurations->logo) }}" />
          ...
          <div class="app-top-bar align-items-center">
              <div class="row app-brand">
                  ...
                  @if(isset($site_configurations) && $site_configurations->logo != "" && file_exists(public_path("uploads/logo/".$site_configurations->logo)))
                          <img src="{{ asset('uploads/logo/'.$site_configurations->logo) }}"/>
                      @else
