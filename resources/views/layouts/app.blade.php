@php 
$allActiveCommunities = App\Models\Community::where('status', 1)->orderBy('sort', 'asc')->get();
@endphp  
<!DOCTYPE html>
<html lang="en-us">
    <head>
    <?php $site_configurations = \DB::table('site_configurations')->where('status',1)->first();?>
        ...
        @if(request()->route() != null  && request()->route()->getName() == 'home')
            @if($site_configurations && $site_configurations->site_title != "")
              {{$site_configurations->site_title}}                
            @endif
        @else
           @yield('title')
        @endif
        ...
        <meta http-equiv="Content-Security-Policy" content="default-src gap://ready file://* *; 
        style-src 'self' http://* https://* 'unsafe-inline'; script-src 'self' http://* https://* 'unsafe-inline' 'unsafe-eval';img-src 'self' blob: data:;">      
        ...
        <link rel="stylesheet" href="{{ asset('css/toggled-search-bar.css') }}">
        <script>
           // set website url for ajax call
            var WEBSITE_URL = "{{ config('app.url') }}";
        </script>
        @php 
            if($_SERVER['SERVER_NAME'] == 'www.databroker.global') {
        @endphp        
        ...
        <script>
          ...
          })(window,document,‘script’,‘dataLayer’,‘GTM-5ZBV9DD’);
        </script>
        ...
        .dropdown-submenu {
          ...
          .fixed_bg .dropdown-toggle:after {
                content: "";
                border: solid white;
    ...
    <body>    
    <?php if($_SERVER['SERVER_NAME'] == 'www.databroker.global') { ?> 
        <!-- Google Tag Manager (noscript) -->
            <noscript><iframe src=“https://www.googletagmanager.com/ns.html?id=GTM-5ZBV9DD”
    ...
    <?php }?> 
        ...
        <nav class="fixed_bg">
        	<div class="navbar navbar-expand-lg app-top-nav-bar-container">
                ...
                <div class="col-auto p-0 col-md-7 col-sm-5">
                    <ul class="navbar-nav left">
                      <!-- <li class="nav-item" style='padding-right:20px' id="top-nav-item"><a class="nav-link"  href="{{ route('about.about') }}">{{ trans('home.about_databroker') }}</a></li> -->
                      ...
                      <li class="nav-item dropdown" id="top-nav-item-dropdown">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="more_dropdown">
                          <li><a class="dropdown-item" href="{{ route('about.about') }}"> {{ trans('home.about_databroker') }} </a></li>
                          <li><a class="dropdown-item" href="{{ route('about.partners') }}"> {{ trans('home.partners') }} </a></li>
                          <li><a class="dropdown-item" href="{{ route('about.matchmaking') }}"> {{ trans('home.datamatch') }} </a></li>
                ...
                <div class="col-auto col-sm-5 col-md-3 p-0">
                      @if( Auth::user() )
                      <ul class="navbar-nav right right-bar">			        		
                        <li class="nav-item" id="auth-top-nav" style='display:none'>
                          <a class="nav-link" href="{{ route('account.myaccount') }}"  aaria-haspopup="true" aria-expanded="false"><i class="icon material-icons mdl-badge">person</i><!-- <i class="icon material-icons mdl-badge" data-badge="!">person</i> -->{{ trans('home.account') }}</a>
                        </li>
                        <!-- LOGOUT -->
                        <li class="nav-item dropdown" id="auth-top-nav" style='display:none'>
                        ...
                        <li class="nav-item dropdown" id="auth-top-nav" >
                          ...
                          <div class="dropdown-menu" aria-labelledby="account_dropdown">                                        
                            ...
                            @php
                              ...
                              $unviewd_bids = \App\Models\Bid::where('userIdx', Auth::user()->userIdx)->where('isBuyyerViewed',false)->get();
                              ...
                              $providers =  DB::table('productshares')
                                    ...
                                    ->where('orgusers.orgUserEmail',Auth::user()->email)
                                    ->groupBy('orgusers.orgIdx')->get();
                            @endphp
                              @if(count($bids)>0 || count($purchases) > 0 || count($providers) > 0)
                                <h5> {{ trans('home.me_buyer') }} </h5>
                              @endif
                              @if(count($bids))
                              <a class="dropdown-item" href="{{ route('profile.buyer_bids') }}"> {{ trans('home.bids_sent') }} 
                                <span class="badge badge-pill badge-danger" style="float:right;margin-bottom:-10px;">{{count($unviewd_bids)}}</span>
                              </a>
                              @endif
                              @if(count($purchases))
                              ...
                              @endif
                              @if(count($providers))
                               <a class="dropdown-item" href="{{ route('my_data_providers') }}"> {{ trans('home.my_data_providers') }} </a>   
                              @endif
                              @if(count($bids)>0 || count($purchases) > 0 || count($providers) > 0)
                                <div class="dropdown-divider"></div>
                              @endif
                            ...
                            @php
                              ...
                              $products = \App\Models\OfferProduct::join('offers', 'offers.offerIdx', '=', 'offerProducts.offerIdx')
                               ...
                               $unviewd_bids = \App\Models\OfferProduct::join('offers', 'offers.offerIdx', '=', 'offerProducts.offerIdx')
                                                                  ->join('providers', 'providers.providerIdx', '=', 'offers.providerIdx')
                                                                  ->join('users', 'users.userIdx', '=', 'providers.userIdx')
                                                                  ->join('bids', 'bids.productIdx', '=', 'offerProducts.productIdx')
                                                                  ->where('users.userIdx', Auth::user()->userIdx)
                                                                  ->where('bids.bidStatus', 0)
                                                                  ->where('bids.isSellerViewed', 0)
                                                                  ->get();                                                                
                            ...
                            @if(count($products))
                              <a class="dropdown-item" href="{{ route('my_private_data_network') }}"> {{ trans('home.my_private_data_network') }} </a>
                              <a class="dropdown-item" href="{{ route('profile.seller_bids') }}"> {{ trans('home.bids_received') }}
                                <span class="badge badge-pill badge-danger" style="float:right;margin-bottom:-10px;">{{count($unviewd_bids)}}</span>
                              </a>
                      ...
                      @else
                        <div class="right right-bar" id="top-nav-right" ><a class='text-decoration' href="{{ route('login') }}">{{ trans('auth.login') }}</a> or
                           <a class='text-decoration' href="{{ route('register') }}">{{ trans('auth.register') }}</a>
                        </div>  
                      @endif
                       
                </div>
                <div class="col-xs-1 col-md-2 col-sm-2 p-0">
                        <div id="navbar" class="floting-search">
                          <div>
                                <a href="#search" class="search-form-tigger btn-search-cloase-icon" 
                                 data-toggle="search-form"><i class="fa fa-search" aria-hidden="true"></i></a>
                       ...
                       <!-- global search form -->
                      <div class="search-form-wrapper search-container">
                        <form class="search-form" id="" method='get' action="{{ route('category_search')}}">
                            @csrf
                            <div class="input-group">
                              <?php 
                              $curSearchKey = '';

                                if(\Route::current()->getName() == 'category_search'){
                                  $curSearchKey = session('curSearchKey')?session('curSearchKey'):'';
                                }
                              ?>
                              <input type="text" name="search_key" class="search form-control search-input" value="<?=$curSearchKey;?>" placeholder="Search here" required>
                              <button type='submit' class='btn btn-sm submit-btn'><i class="fa fa-search fa-mysearch" aria-hidden="true"></i></button>
                              <!-- <span type='submit' class="input-group-addon" id="basic-addon2"><i class="fa fa-search" aria-hidden="true"></i>
                              </span> -->                              
                              <span class="input-group-addon search-close" id="basic-addon2">
                              <a href="#search"><i class="fa fa-times search-close-icon" aria-hidden="true"></i></a>
                              </span>
                            </div>
                        </form>
                      </div>
                ...
                <div class="row app-brand">                           
                    <a href="{{route('home')}}">
                      @if(isset($site_configurations) && $site_configurations->logo != "" && file_exists(public_path("uploads/logo/".$site_configurations->logo)))
                          <img src="{{ asset('uploads/logo/'.$site_configurations->logo) }}" id="top-bar-img"/>
                      @else
        ...
        <div class="container-fluid app-nav-bar-container">        
        	<nav class="navbar  bg-white navbar-expand-lg" id="main_menu">
              ...
              <div class="row app-brand">
          			<a href=" {{route('home') }}">
                  <img alt="Databroker Global" src="{{ asset('/images/logos/site_logo_new.png') }}" id="top-bar-img"/>                                                
                </a>    
              </div>

              <!-- <h4>Categories <i class="fa fa-arrow-right" aria-hidden="true"></i> </h4> -->

			        <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNavDropdown" 
                    aria-controls="navbarNavDropdown" aria-expanded="false" 
                    aria-label="Toggle navigation" id="top-bar-menu-btn">
                ...
                @php
                  $comm_slug = $comm->slug;
                  $route =  route("data_community.$comm_slug");
                @endphp

                <li class="dropdown nav-item">

    			          <a href="{{$route}}" class="nav-link"><span>{{$comm->communityName}}</span><i class="material-icons">chevron_right</i></a>
  			          </li>
                  @endforeach 
        ...
        <dir class="cookie-accept">
          <div class="container">
            <div class="cookie-section">
              <div class="cookie-logo"><img alt="Databroker Global" src="{{asset('/images/patterns/graylogo.svg')}}"/></div>
              <div class="row">
      ...
      <div id="bodyContent">
        @yield('content')
  		...
  		<div class="container-fluid app-wapper">
  			...
  			<div class="container footer_section1">
                ...
                $route =  route("data_community.$comm_slug");
                @endphp
                <li><a href="{{$route}}">{{$comm->communityName}}</a></li>
                @endforeach               
                </ul>
        ...
        <div class="container-fluid app-wapper footer-section">
          <div class="container" id="footer-section-container">
            <div class="row app-section  app-footer-section align-items-center">
              <div class="app-footer-link col-md-12 col-lg-4">
              ...
              <div class="site_footer_logo_container col-md-12 col-lg-4">
                @if(isset($site_configurations) && $site_configurations->footer_logo != "" && file_exists(public_path("uploads/logo/".$site_configurations->footer_logo)))
                <img width="300px" src="{{ asset('uploads/logo/'.$site_configurations->footer_logo) }}">
                @else
              ...
              <div class="app-footer-social-link-container col-md-12 col-lg-4">                      
                <h4 class="text-follow-footer">Follow us on social media</h4>
                <div class="app-footer-social-link">
                  <a href="{{env('FACEBOOK_LINK')}}" rel="nofollow noopener noreferrer" target="_blank">
    ...
    <script src="{{ asset('js/toggled-search-bar.js') }}"></script>
