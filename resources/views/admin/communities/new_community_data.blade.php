    ...
    <div class="m-subheader ">
        <div class="d-flex align-items-center">
            <div class="mr-auto width-100">
                <h3 class="m-subheader__title m-subheader__title--separator"><b style="color: #9102f7;">New In The Communities</b></h3>
                <div class="ggn-r">
                    <a href="{{ route('admin.community.data.new.add') }}" class="btn btn-primary text-right">Add New In The Community</a>
                </div>
            </div>
        </div>
    </div>
                            ...
                            @foreach($communitiesData as $index=>$communityData)
                                <tr>
                                    ...
                                    <a target="_blank" href="{{ route('admin_data_details',[str_replace(' ', '', strtolower($communityData->offer->provider->companyName)),str_replace(' ', '-', strtolower($communityData->offer->offerTitle)).'-'.$offer_region]) }}" class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill" title="View offer details">
                                        ...
                                        <a href="{{route('admin.community.data.new.edit', $communityData->id)}}" class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill" title="Update">
...
<script src="{{ asset('adminpanel/assets/vendors/custom/datatables/datatables.bundle.js') }}"></script>
    ...
    <script src="{{ asset('adminpanel/js/new-community-data.js') }}"></script>
@endsection
