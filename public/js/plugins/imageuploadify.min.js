...
(function($, window, document, undefined) {
    ...
    const compareMimeType = function(mimeTypes, fileType, formatFile) {
    ...
    $.fn.imageuploadify = function(opts) {
        ...
        this.each(function() {
            ...
            let dragbox = $('<div class="imageuploadify well"><div class="imageuploadify-overlay"><i class="fa fa-picture-o"></i></div><div class="imageuploadify-images-list text-center"><span class="imageuploadify-message">Drag and drop '+fileType+' here</span><p>OR</p><button type="button" class="secondary-btn w225 mgh25">'+title+'</button></div><div class="description hide" /><div class="imageuploadify-image-block hide"></div></div>');
            ...
            const retrieveFiles = function(files, input_name) {
                let exist_files = totalFiles[input_name];               
                if((exist_files != undefined && exist_files.length > 0) || files.length > 0 )
                    hideDummyZone();                
                ...
                for (let index = 0; index < files.length; ++index) {                         
                    if (!accept || compareMimeType(acceptlist, files[index].type, /(?:\.([^.]+))?$/.exec(files[index].name)[1])) {
                        ...
                        let only_accept = "";
                        for(let i = 2; i<acceptlist.length;i++){
                            only_accept +=acceptlist[i]+", ";
                            i = i+2;
                        }
                        only_accept = only_accept.slice(0, -1)
                        alert('Please upload '+only_accept+' only!');
                        $('.data-offer .imageuploadify .imageuploadify-images-list').css('display','block');
                    }
                }
            }

            // Function to read a file.
            const readingFile = function(id, file) {
            ...
            const appendFileContainer = function(id, filetype, content) {
                ...
                let deleteBtn = $('<i class="fa fa-times remove icon-delete"></i>');
                let container = $('<div class="imageuploadify-container"><!--<div class="imageuploadify-details"><span>'+content+'</span><span>'+filetype+'</span></div>--></div>'
                  );
                ...
                if ($(self).attr("filelist") === undefined) {
                    ...
                    container.css('background-position', 'center 90%');
            ...
            const enableMouseEvents = function() {
            ...
            const prepareRemoteFilesPreview = function() {
            ...
            const previewMediaLibraryImage = function(img) {
                if (!img) {
                    return;
                }
                        ...
                        for (var i = 0; i < files.length; i++) {	
                            ...
                            if (file.origin === 'remote') {
                                if (file.remotefiletype === 'gallery') {
                                    formData.append('gallery_'+input_name+'_'+y, file.file);
                                } else {
                                    break;
                                }
                            }
    						formData.append(input_name+'_'+y, file.file);
                        }
                        let removes = removedFiles[input_name];
                        if (removes !== undefined) {
                            for (var j = 0; j < removes.length; j++) {
								var z = j+1;
                                formData.append('removed_'+input_name+'_'+z, removes[j].filename);
                            }
                        }
                    }
