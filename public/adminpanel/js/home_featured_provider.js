            ...
            columnDefs: [
                {
                    ...
                    //     return `
                    //         <a href="`+ WEBSITE_URL+`/admin/home_featured_provider/edit/`+data+`" class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill" title="Update">
                    //         <i class="la la-edit"></i>
                    //         </a>
                    //         <a href="javascript:void(0);" class="btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill" onclick="return wantDelete('`+data+`');"><i class="la la-trash" title="Delete"></i>
                    //         </a>`;
                    // },
                },
                {
...
function publish_record(record_idx){
    $.ajax({
      ...
      url: WEBSITE_URL+'/admin/home_featured_provider/publish',
      data: {id: record_idx},
      method: 'post',
...
$("#upload_attach").change(function(event){
    ...
    $.ajax({
        ...
        url: WEBSITE_URL+"/admin/home_featured_provider/upload_attach/" + attach_record_idx,
        type: 'POST',
        data: data,
        contentType: false,
        processData: false,
    ...
    function(isConfirm) {
      if (isConfirm) {
        $.ajax({
          ...
          url: WEBSITE_URL+'/admin/home_featured_provider/delete/'+record_idx,
          method: 'post',
          success: function(res){
            if(res=="success"){
              window.location.href= WEBSITE_URL+"/admin/home_featured_provider";
