                ...
                },
                {
                    ...
                    render: function(data, type, full, meta) {
                      ...
                      if(full[7] == "Published") {                        
                        ...
                        return publish_icon + '<a href="'+WEBSITE_URL+'/admin/updates/edit/' + data + '" class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill" title="Update"><i class="la la-edit"></i></a><a href="#" class="btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill" onclick="wantDelete(\''+data+'\');"><i class="la la-trash" title="Delete"></i></a>';
                    },
                },
            ],
        };
...
function publish_record(record_idx){
  $.ajax({
    ...
    url: WEBSITE_URL+'/admin/updates/publish',
    data: {articleIdx: record_idx},
    method: 'post',
    success: function(res){
      if(res=="success"){
        window.location.href= WEBSITE_URL+"/admin/updates";
      }
    }
  });
}
  ...
  function(isConfirm) {
    if (isConfirm) {
      $.ajax({
        ...
        url: WEBSITE_URL+'/admin/updates/delete/'+record_idx,
        method: 'post',
        success: function(res){
          if(res=="success"){
            window.location.href= WEBSITE_URL+"/admin/updates";
